...
// Add by syslot
bool PhysicsDirect::getJointInfoPhysX(int bodyIndex, int jointIndex, struct b3JointInfo& info) const
{
      PhysXURDFImporter *u2p = *(static_cast<PhysXServerCommandProcessor*>(m_data->m_commandProcessor))->extraUrdfs.find(btHashInt(bodyIndex));
	...
	if (bodyJointsPtr && *bodyJointsPtr)
	{
		...
		if ((jointIndex >= 0) && (jointIndex < bodyJoints->m_jointInfo.size()))
		{
			...
			::strcpy(info.m_jointName, u2p->getLinkName(jointIndex).c_str());
