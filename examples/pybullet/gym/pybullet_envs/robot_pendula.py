...
class InvertedPendulum(URDFBasedRobot):
    ...
    def robot_specific_reset(self, bullet_client):
        ...
        #     current_pos = self.parts[key].current_position()
        #     des_pos = copy.copy(current_pos)
        #     des_pos[1]=self.pos[1]
        #     self.parts[key].reset_position(des_pos)
        
        # pos, orn = self._p.getBasePositionAndOrientation(self.pole)
        # print("pos: ", pos, ", orn: ", orn)
        # self._p.resetBasePositionAndOrientation(self.pole, self.pos, [0,0,0,1])
...
class InvertedDoublePendulum(URDFBasedRobot):
    def __init__(self, basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], self_collision=False, fixed_base=False, isPhysx=False):
