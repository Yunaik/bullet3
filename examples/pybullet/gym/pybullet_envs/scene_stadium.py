...
class StadiumScene(Scene):
    ...
    def episode_restart(self, bullet_client):
        ...
        if (self.stadiumLoaded==0):
            ...
            # filename = os.path.join(pybullet_data.getDataPath(),"plane100.urdf")
            # self.ground_plane_mjcf=self._p.loadURDF(filename)
            # #filename = os.path.join(pybullet_data.getDataPath(),"stadium_no_collision.sdf")
            # #self.ground_plane_mjcf = self._p.loadSDF(filename)
            # #    
            # # for i in self.ground_plane_mjcf:
            # self._p.changeDynamics(ground_plane_mjcf,-1,lateralFriction=0.8, restitution=0.5)
            # self._p.changeVisualShape(ground_plane_mjcf,-1,rgbaColor=[1,1,1,0.8])
...
class MultiplayerStadiumScene(StadiumScene):
    ...
    #     # print("Introducing %d" % robot.player_n)
